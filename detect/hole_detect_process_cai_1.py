...
class HoleDetectConvector:
    ...
    @staticmethod
    def draw_labels(frame: np.ndarray, hole_details: dict, now_hole: int = -1, debug=False) -> np.ndarray:
        frame = frame.copy()
        x_move = 25
        y_move = -25
        lw = max(round(sum(frame.shape) / 2 * 0.003), 2)
        for _, value in hole_details.items():
            tag = value['tag']
            if 'coordinate_real' in value and not debug:
                coordinate = value['coordinate_real']
            else:
                coordinate = value['coordinate']
            if now_hole == -1 or int(tag) == now_hole:
                tag_str = tag.zfill(2)
                if value['status'] == 'hole':
                    color = (50, 108, 66)
                elif value['status'] == 'hole_stage2':
                ...
                cv2.rectangle(frame, (int(coordinate['left_top'][0]), int(coordinate['left_top'][1])),
                              (int(coordinate['right_bottom'][0]), int(coordinate['right_bottom'][1])), color, 2)
                p1 = (int(coordinate['middle'][0]) + x_move, int(coordinate['middle'][1]) + y_move)
                tf = max(lw - 1, 1)
                w, h = cv2.getTextSize(tag_str, 0, fontScale=lw / 3, thickness=tf)[0]
                outside = p1[1] - h >= 3
                p2 = p1[0] + w, p1[1] - h - 3 if outside else p1[1] + h + 3
                if not debug:
                    cv2.rectangle(frame, p1, p2, color, -1, cv2.LINE_AA)
                    cv2.putText(frame, tag_str, (p1[0], p1[1] - 2 if outside else p1[1] + h + 2), 0,
                                lw / 3, (255, 255, 255), thickness=tf, lineType=cv2.LINE_AA)
        return frame
