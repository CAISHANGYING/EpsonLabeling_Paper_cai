...
class StandWorkpiece:
    ...
    def get_hole_order(self) -> list:
        """
        Returns a sorted list of hole IDs based on the tags for defining a specific order.
        """
        # 將孔洞按 `tag` 值排序後返回
        return sorted(self._holeDetail.keys(), key=lambda x: self._holeDetail[x]["tag"])
